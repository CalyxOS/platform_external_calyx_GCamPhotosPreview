...
object IntentHandler {
    ...
    fun rewriteIntent(intent: Intent): Intent {
        ...
        val clipData = if (isSecure(intent)) {
            ...
            val secureIds = intent.extras?.getSerializable(EXTRA_SECURE_IDS, LongArray::class.java)?.filter { id ->
                id != mainId // don't include main Uri again
            }
            if (secureIds.isNullOrEmpty()) {
                null
            } else {
                // clip data needs to be constructed with first Uri
                val clipDataUri = ContentUris.withAppendedId(FILES_EXTERNAL_CONTENT_URI, secureIds[0])
